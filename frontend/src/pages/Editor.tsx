...
import BomModal from '../components/Editor/BomModal';
import ReportSettingsModal from '../components/Editor/ReportSettingsModal';
import { useEditorStore } from '../stores/editorStore';
import { exportCanvasToPDF, exportCanvasToPNG } from '../utils/pdfExport';
import { exportDocumentationToMarkdown } from '../utils/docExport';
...
export default function Editor() {
  ...
  const handleExportMarkdown = () => {
    setIsExportOpen(false);
    exportDocumentationToMarkdown(projectName);
  };
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-slate-100 text-slate-900 overflow-hidden font-sans">
      ...
      <header className="h-14 bg-white/90 backdrop-blur-xl border-b border-slate-200/70 flex items-center justify-between px-4 shadow-sm z-30 relative flex-shrink-0">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors text-sm font-medium" 
            title="Nomenclature & Coûts"
          >
            <Layers className="w-4 h-4" />
            <span className="hidden xl:inline">BOM</span>
          </button>
      ...
      <ReportSettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />
